...
RSpec.describe Api::V1::Admin::User::SkillsController, type: :controller do
  ...
  describe "GET #create" do
    it "creates skill rate for a user" do
      ...
      expect{
            post :create, format: :json, params: { user_skill: { user_id: user.id, skill_id: skill.id, rate: 10 } }
    ...
    it "won't create duplicate skill rate" do
      ...
      expect{
            post :create, format: :json, params: { user_skill: { user_id: user.id, skill_id: skill.id, rate: 5 } }
  ...
  describe "DELETE #destroy" do
    it "destroys skill rate" do
      ...
      UserSkill.create(user_id: user.id, skill_id: skill.id, rate: 10)
      expect{
            get :destroy, format: :json, params: { id: user.id, user_skill: { user_id: user.id, skill_id: skill.id } }
          }.to change(UserSkill, :count).by(-1)
    end
  end
end
